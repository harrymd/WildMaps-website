...
export default function useMap(containerRef, currentBasemapStyle) {
  ...
  //const [currentStyle, setCurrentStyle] = useState('style/liberty_underlay.json');

  useEffect(() => {
    if (mapRef.current || !containerRef.current) return;

	console.log(currentBasemapStyle);
  ...
  }, [containerRef, currentBasemapStyle]);

  // Change the basemap style.
  useEffect(() => {
    if (!mapRef.current) return;
  
    mapRef.current.setStyle(currentBasemapStyle);
