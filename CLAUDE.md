...
- **`SURVEY_FORM_VERSION`** (`constants/methodologicalStandards.ts`, currently `'2.1'`) is sent as a hidden `form_version` field in every submission payload — bump it if the question set changes again.
- **`STANDARD_GUIDANCE_ROWS`** (`constants/methodologicalStandards.ts`) is the Gold/Silver/Bronze practitioner-guidance table, rendered via the shared `StandardsGuidanceTable` component — as a blocking pop-up (`StandardsGuidanceModal`, closable only via its 'X') from an info button next to the 'Standard' column header on `SelectDataset`, and inline in `MethodStandardScoreSection` on `FinalScreen`. Keep the scoring cutoffs in `calculateChecklistResult` (currently gold ≥80%, silver ≥60%) in sync with this table's score ranges if either changes.
